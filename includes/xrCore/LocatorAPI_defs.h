...
struct XRCORE_API FS_File{
	...
	time_t	  	time_write;
	long     	size;
	xstring	name;			// low-case name
	void		set			(xstring nm, long sz, time_t modif,unsigned attr);
public:
				FS_File		(){}
				FS_File		(xstring nm);
				FS_File		(const _FINDDATA_T& f);
				FS_File		(xstring nm, const _FINDDATA_T& f);
				FS_File		(xstring nm, long sz, time_t modif,unsigned attr);
