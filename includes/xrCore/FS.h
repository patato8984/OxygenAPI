...
XRCORE_API void createPath(const char* path, bool bIsFileName = false, bool bIsAbsolute = false);
...
public:
    ...
    void w_stringZ(const xstring& p)
	{
        w(p.c_str(), p.size());
        w_u8(0);
    }
...
public:
    ...
    void r_string(xstring& dest);

    void skip_stringZ();

    void r_stringZ(char* dest, const size_t tgt_sz);
    void r_stringZ(shared_str& dest);
    void r_stringZ(xstring& dest);
