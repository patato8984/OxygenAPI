//////////////////////////////////////////////////////////
// Desc   : Oxygen string classes : xstring (std::string) / shared_str
// Author : Im-Dex, ForserX, Giperion
//////////////////////////////////////////////////////////
...
public:
	...
	// construction
			constexpr	shared_str	() : p_(nullptr)						{ }
						shared_str	(const char* rhs) : p_(nullptr)				{	_set(rhs); }
						shared_str	(shared_str const &rhs) : p_(nullptr)	{	_set(rhs); }
						~shared_str	()										{	_dec();	   }

	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// assignment & accessors
	shared_str&			operator=	(const char* rhs)						{	_set(rhs);	return *this;		}
...
class XRCORE_API xstring : public std::basic_string<char, std::char_traits<char>, xalloc<char>>
{
public:
	typedef std::basic_string<char, std::char_traits<char>, xalloc<char>> Super;

	xstring(const char* Str);
	xstring(const char* Str, u32 Size);
	xstring(const xstring& other);
	xstring(const xstring&& other);
	xstring(const Super&& other);
	xstring();

	xstring& operator=(const char* Str);
	xstring& operator=(const xstring& other);
	xstring& operator=(const Super& other);
	xstring& operator=(const std::string_view& other);

	template <size_t ArrayLenght> xstring(char* (&InArray)[ArrayLenght])
	{
		assign(InArray, ArrayLenght);
	}

	xvector<xstring> Split(char splitCh);
	xvector<xstring> Split(u32 NumberOfSplits, ...);

	bool StartWith(const xstring& Other) const;
	bool StartWith(const char* Str) const;
	bool StartWith(const char* Str, size_t Size) const;
	xstring RemoveWhitespaces() const;

	static xstring ToString(int Value);
	static xstring ToString(unsigned int Value);
	static xstring ToString(float Value);
	static xstring ToString(double Value);
	static xstring ToString(const Fvector& Value);
	static xstring ToString(const Dvector& Value);

	using xrStringVector = xvector<xstring>;
	static xstring Join(xrStringVector::iterator beginIter, xrStringVector::iterator endIter, const char delimeter = '\0');
	...
	template<> void FixSlashes<xstring>(xstring& InStr)
	{
...
using SStringVec = xvector<xstring>;
...
IC void	xr_strlwr(xstring& src)								  { for(char & it : src) it=xstring::value_type(tolower(it));}
...
namespace std
{
	...
	inline xstring to_xstring(u64 value)
	{
		const char* HashString = xr_strdup(to_string(value).c_str());
		return HashString;
	}
}
#pragma pack(pop)